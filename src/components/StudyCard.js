import { Study } from "../contents"
const StudyCard = () => {
  return (
    <div className="p-6">
            <h3 className="text-base uppercase text-white font-semibold mb-6">Education</h3>
            <ul>
                {Study.map((item, index) => (
                    <li className="relative pb-8 last:pb-0 pl-6 after:inline-block after:absolute after:top-8 
                        ...
                        <p className="text-2xl text-white font-semibold mb-1">{item.degree}</p>
                        <p>
                            <a className="text-amber-400 cursor-pointer" href={item.website} rel="noreferrer" target="_blank">{item.institute}</a> 
